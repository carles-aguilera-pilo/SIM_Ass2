...
There are 851 extreme multivariate outliers in the dataset that could negatively affect the model estimates and predictions. These outliers represent observations with unusual combinations of predictor values that do not conform to the general pattern of the data. Rather than removing them outright, we will flag these observations by creating a binary variable called isOutlier.
...
The relationship between training hours and the probability of the target is approximately linear and negative across all company size groups. While baseline probabilities differ by company size, the slopes of the curves are very similar, indicating that the effect of training hours is largely consistent across groups. The absence of curve intersections suggests that the interaction between training hours and company size is weak, and that the main effects capture most of the observed variation.
...
# Interaction training_hour with company_size
plot(effect(term = "company_size_group:training_hours",mod = mdl_f,xlevels = list(training_hours = seq(min(df$training_hours),max(df$training_hours),length = 100),company_size_group = levels(df_train$company_size_group))),multiline = TRUE,ci.style = "none",main = "Effect of Training Hours by Company Size")
