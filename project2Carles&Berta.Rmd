...
m0 <- glm(target~f.city_development_index+f.experience+f.training_hours+last_new_job, family = binomial, data = df_train)
m0v1 <- glm(target~f.city_development_index+f.experience_grouped+f.training_hours+last_new_job, family = binomial, data = df_train)
summary(m0); summary(m0v1)
residualPlots(m0); summary(m0)
...
mdl_f <- glm(target ~ poly(city_development_index, 4)*(relevent_experience+enrolled_university+education_level+company_size+company_type) + poly(experience, 2) * education_level +training_hours +gender +company_size + company_type, family=binomial(), data=df_train)
...
# WE CAN ALSO DELETE -> 15320, 16177 (CONSIDERAR AQUESTS).
...
```{r}
hoslem.test(df_test$target, pred_class)
```