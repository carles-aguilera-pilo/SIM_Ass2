...
```{r}
df_numeric <- df[, c("city_development_index", "training_hours", "experience")]
...
## Variables study: quality report
...
## Individuals study: quality report

In this section, we analyze the additional variables that were created in order to better understand the behavior of the observations in our dataset.
...
With respect to the isOutlier variable, approximately 4.5% of the observations are identified as outliers, either due to the numerical variables city_development_index or training_hours. In contrast, the variable experience does not present outliers. The remaining observations, approximately 95.5%, are therefore considered non-outliers.
...
After analyzing the behavior of these variables, a new variable called allErrors is created. This variable summarizes, for each observation, the total number of detected issues by combining indicators of missing values (isNA), univariate outliers (isOutlier), and multivariate outliers (isMultivariateOutlier). 
...
AIC(mdl221, mdl22) # It continues to being better the mdl22, it's the one that has the lowest AIC.
```

It can also be seen that company_size categorical variable could be grouped, as several of its categories are not statistically significant. Therefore, these categories are combined into a single group representing companies with fewer than 10.000 employees.

In this case, the AIC statistic reveals that taking the variable in its grouped form improves the model.
...
AIC(mdl222, mdl22) # The best is mdl222
...
Company_type is another variable that could potentially be grouped. However, the proposed groups based on their statistical measures do not seem coherent, and there are indications that such grouping could lead to overfitting.

On the other hand, last_new_job and education_level do not seem suitable for grouping, as all their categories show differentiated behavior and contribute relevant information to the model.
...
Finally, when computing the statistical tests for the regression coefficients, specifically testing the null hypothesis that each coefficient is equal to zero conditional on the rest of the variables being included in the model, it can be observed that the interactions terms poly(city_development_index, 4):relevant_experience and company_size_group:poly(experience, 2) are not statistically meaningful. Consequently, this interaction was removed from the model, even though the AIC statistic did not show a substantial improvement.
...
Hence, the final model selected has been:  target ~ poly(city_development_index, 4) * (enrolled_university + education_level + company_size_group + company_type + last_new_job) + poly(experience, 2) * (company_type + last_new_job) + training_hours + relevent_experience
...
```{r}
mdl_f <- glm(target ~ poly(city_development_index, 4) * (enrolled_university + education_level + company_size_group + company_type + last_new_job) + poly(experience, 2) * (company_type + last_new_job) + training_hours + relevent_experience, family=binomial(), data=df_train)
...
Anova(mdl_f, test = "LR") # all coefficients are statistically meaningful
```
...
mdl_f <- glm(target ~ poly(city_development_index, 4) * (enrolled_university + education_level + company_size_group + company_type + last_new_job) + poly(experience, 2) * (company_type + last_new_job) + training_hours + relevent_experience, family=binomial(), data=df_train_clean)
...
# 1 observation should be removed
...
idx_cooks <- which(cook_influential > 0.02); length(idx_cooks);
# 1 observation should be removed
...
After removing the influential observations, there is still some observation that highly influences the model and should be removed.
...
It is important to highlight that the influential observations do not follow any concerning or systematic pattern, except that they were all classified as multivariate outliers.
...
*relevent_experience*
Belonging to the category "no relevent experience" is associated with an increase in the odds of the positive outcome of approximately 23.29% compared to the reference category "relevent experience", all else being equal.

```{r}
val <- exp(coef(mdl_f)["relevent_experienceNo relevent experience"]); val
(val-1)*100

plot(eff$relevent_experience,
     multiline=TRUE,
     main="Effect of Relevent experie")
```

*training_hours*
An increment of one unit in training_hours is associated with a decrese n the odds of the positive outcome of approximately 0.1%, all else being equal.

```{r}
val <- exp(coef(mdl_f)["training_hours"]); val
(1-val)*100
plot(eff$training_hours,
     multiline=TRUE,
     main="Effect of Training Hours")
```

*Other variables*

The rest of categories interpretation depend on the category level, the city development index and/or the experience values since there are interactions between variables.

Hence, it is better to interpret these variables using all effects plots.

For all last_new_job categories, individuals are much more likely to be job-seeking in less developed cities, while their probability drops sharply as the city_development_index increases. Although all categories reach an initial minimum around city development indexes values 0.75-0.8, their behavior begins to diverge substantially at higher development levels. In particular, the category for last new job being never stands out for displaying a markedly different pattern compared with the rest. These discrepancies confirm that the interaction between last_new_job and city_development_index is meaningful and captures important heterogeneity in the relationship.
...
Except for Phd student, the remaining categories seem to follow a similar pattern, showing comparable trends in how the city_development_index relates to the target probability. However, at high levels of city development, the probabilities across categories tend to converge.

For both the more than 10000 and less than 10000 employees categories, the effect of the city_development_index is strongly non-linear: the predicted probability increases sharply at low city_development_index values, reaches a maximum around 0.58-0.60, and then gradually decreases in highly developed cities. Nonetheless, both functions show intercepts, indicating that the interaction between company sized and city development is indeed meaningful. Note that category Unknown follows a complete different pattern.

The relationship between city_development_index and the target probability is particularly striking. All company_type categories start with relatively high probabilities of being job-seeking in less developed cities and show a marked decline as the city development index increases. However, the Startup and Other categories display noticeably different and more irregular patterns, standing out from the overall trend and deviating more strongly from the rest. This reinforces the relevance of the interaction between company_type and city_development_index in explaining job-seeking behavior.

The relationship between experience and the target probability strongly depends on the company_type category. No clear or common pattern emerges across groups, as the curves intersect at different points. This highlights the importance of the interaction between experience and company type in capturing heterogeneous job-seeking behavior.

Finally, notice that for both the full time and no_enrollment categories, the effect of the city_development_index is strongly non-linear: the predicted probability increases sharply at low city_development_index values, reaches a maximum around 0.58-0.60, and then gradually converges to a constant probability. Nonetheless, both functions show intercepts, indicating that the interaction between company euniversity enrollment and city development is indeed meaningful. Note that category part time course follows a complete different pattern.
...
# Interaction experience with company_type
plot(effect(term = "company_type:poly(experience,2)", mod = mdl_f, xlevels = list(experience = seq(min(df$experience), max(df$experience), length = 50))),multiline = TRUE,ci.style = "none",main = "Effect of Experience by Company Type")

# Interaction city_development_index,4 with enrolled_university
plot(eff$`poly(city_development_index,4):enrolled_university`,
     multiline=TRUE,
     main="Effect of City Development by Enrolled to university")
```
